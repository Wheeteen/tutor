  ...
  Vue.http.interceptors.push(function(request, next){
	...
	var vm = new Vue({
      ...
      methods: {
        ...
        getParam: function() {   //获取url上的参数
          var str = location.search;
          if (str.indexOf("?") != -1&& str.indexOf("=") == -1){
            this.status.isReturn = true;
            this.url = 'createParentOrder';
          }
          else{
            str = str.substr(1,str.length).split("=");
            this.id = parseInt(str[1]);
            this.status.isReturn = false;
            this.url = 'updateInfo';
            this.renderData();
          }
        },
        ...
        onSubmitQuestion: function(){
          ...
          //   this.status.isSubmit = true;
            ...
            this.$http.post(this.domain+this.url, json, {
              crossOrigin: true,
              headers:{
                'Content-Type':'application/json' 
              }
              ...
              if (res.json().success == 1) {
                ...
                var url;
                if (this.url == 'updateInfo'){
                  url='./detailedInfoParent.html?listId='+this.id;
                }
                else{
                  url = './userAdministor.html';
                }
